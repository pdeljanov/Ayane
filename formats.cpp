...
static inline [[const]] uint8_t clip_uint8( int in )
{
    if ( in & ( ~0xFF ) )
        return ( -in ) >> 31;
    else
...
static inline [[const]] int8_t clip_int8( int in )
{
    if ( ( in + 0x80 ) & ~0xFF )
        return ( in >> 31 ) ^ 0x7F;
    else
...
static inline [[const]] uint16_t clip_uint16( int in )
{
    if ( in & ( ~0xFFFF ) )
        return ( -in )>>31;
    else
...
static inline [[const]] int16_t clip_int16( int in )
{
    if ( ( in + 0x8000 ) & ~0xFFFF )
        return ( in >> 31 ) ^ 0x7FFF;
    else
...
static inline [[const]] int32_t clip_int24( int in )
{
...
static inline [[const]] int32_t clip_int32( int64_t in )
{
